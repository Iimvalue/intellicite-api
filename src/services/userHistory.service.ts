...
export async function getSingleSearchHistoryService(userId: string, historyId: string) {
  return await UserHistory.findOne({ _id: historyId, userId });
}

// add a new search history entry for a user
export async function addUserSearchHistoryService(userId: string, query: string) {
...
export async function deleteSingleSearchHistoryService(userId: string, historyId: string) {
